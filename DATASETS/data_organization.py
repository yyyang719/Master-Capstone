...
pgroup.drop(['School Name','Low Income #', 'Low Income %','Free Lunch #', 'Free Lunch %', 'Reduced Lunch #','Reduced Lunch %'], axis=1, inplace=True)
...
MCAS_LEA = MCAS[MCAS.Subject=='ELA']
MCAS_MATH = MCAS[MCAS.Subject=='MATH']

columns = MCAS.columns.values
columns_LEA = [name + " LEA" for name in columns[1:]]
columns_MATH = [name + " MATH" for name in columns[1:]]

MCAS_LEA.columns = ["School Code"] + columns_LEA
MCAS_MATH.columns = ["School Code"] + columns_MATH
MCAS_LEA.drop("Subject LEA", axis=1, inplace=True)
MCAS_MATH.drop("Subject MATH", axis=1, inplace=True)

data_2018 = pd.merge(data_2018, MCAS_LEA, how='left', on='School Code')
